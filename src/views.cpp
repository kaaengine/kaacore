...
View::refresh()
{
    ...
    glm::dvec2 drawable_area = static_cast<glm::dvec2>(renderer->view_size);
    glm::dvec2 border_size = static_cast<glm::dvec2>(renderer->border_size);
    glm::dvec2 virtual_resoultion = static_cast<glm::dvec2>(engine->virtual_resolution());
    glm::dvec2 virtual_dimensions = static_cast<glm::dvec2>(this->_dimensions);
    auto view_dimensions = virtual_dimensions / virtual_resoultion * drawable_area;
    auto view_origin = static_cast<glm::dvec2>(this->_origin) / virtual_resoultion * drawable_area;
    // start drawing from edge of border
    auto clipped_view_origin = glm::max(view_origin, 0.);
    ...
    auto projection_factor = clipped_view_dimensions / view_dimensions;

    bgfx::setViewRect(
        this->_index,
        uint16_t(clipped_view_origin.x + border_size.x),
        uint16_t(clipped_view_origin.y + border_size.y),
        uint16_t(clipped_view_dimensions.x),
        uint16_t(clipped_view_dimensions.y)
    );

    glm::fmat4 projection_matrix = glm::ortho(
        -virtual_dimensions.x * projection_factor.x / 2,
        virtual_dimensions.x * projection_factor.x / 2,
        virtual_dimensions.y * projection_factor.y /  2,
        -virtual_dimensions.y * projection_factor.y / 2
    );
...
ViewsManager::operator[](const int16_t z_index)
{
    ...
    auto view_index = z_index + (KAACORE_MAX_VIEWS / 2);
    return this->_views[view_index];
}
