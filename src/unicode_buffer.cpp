...
UnicodeView::UnicodeView()
    : _data(nullptr), _length(0), _representation_size(UnicodeRepresentationSize::ucs1)
{}

UnicodeView::UnicodeView(
    const std::byte* data, const std::size_t length,
    const UnicodeRepresentationSize representation_size)
    : _data(const_cast<std::byte*>(data)),
      _length(length),
...
UnicodeView::UnicodeView(const UnicodeStringViewVariant& view_variant)
{
    std::visit(
        [this](const auto& view) {
            ...
            } else if constexpr (std::is_same_v<T, std::basic_string_view<char16_t>>) {
                this->_representation_size = UnicodeRepresentationSize::ucs2;
            } else if constexpr (std::is_same_v<T, std::basic_string_view<char32_t>>) {
...
UnicodeView::iterator::iterator(const UnicodeView* view, const std::size_t index)
    : _view(view), _index(index)
{}

UnicodeCodepoint UnicodeView::iterator::operator*() const
{
    const auto* ptr = this->_view->_data + this->_index * static_cast<std::size_t>(this->_view->_representation_size);
    if (this->_view->_representation_size == UnicodeRepresentationSize::ucs1) {
        return *reinterpret_cast<const char*>(ptr);
    } else if (this->_view->_representation_size == UnicodeRepresentationSize::ucs2) {
        return *reinterpret_cast<const char16_t*>(ptr);
    } else if (this->_view->_representation_size == UnicodeRepresentationSize::ucs4) {
        return *reinterpret_cast<const char32_t*>(ptr);
    }
    throw kaacore::exception("Invalid Unicode representation size");
}

UnicodeView::iterator& UnicodeView::iterator::operator++()
{
    this->_index++;
    return *this;
}
...
UnicodeView::operator==(const UnicodeView& other) const
{
    ...
    return std::equal(
        this->_data,
        this->_data + (this->_length * static_cast<std::size_t>(this->_representation_size)),
        other._data
    );
}
...
UnicodeBuffer::UnicodeBuffer()
    : _data_container(), _length(0), _representation_size(UnicodeRepresentationSize::ucs1)
{}
...
UnicodeBuffer::UnicodeBuffer(const UnicodeStringViewVariant& view_variant)
{
    std::visit(
        [this](const auto& view) {
            ...
            this->_length = view.size();
            this->_data_container = std::vector<std::byte>(view.size() * static_cast<std::size_t>(this->_representation_size));
            const std::byte* view_data_ptr = reinterpret_cast<const std::byte*>(view.data());
            std::copy(
                view_data_ptr,
                view_data_ptr + (view.size() * static_cast<std::size_t>(this->_representation_size)),
