    ...
    const UnicodeView& codepoints)
{
    ...
    for (UnicodeCodepoint base_codepoint : default_codepoints) {
        auto& sdf = sdfs.emplace_back(font_info, base_codepoint, font_baking_scale);
        KAACORE_LOG_TRACE(
    ...
    for (UnicodeCodepoint additional_codepoint : codepoints) {
        auto& sdf = sdfs.emplace_back(font_info, additional_codepoint, font_baking_scale);
        KAACORE_LOG_TRACE(
            "Generated SDF glyph for additional codepoint: {:#x}, dimensions: ({}, {}), "
            "offset: ({}, {})",
            additional_codepoint, sdf.dimensions.x, sdf.dimensions.y, sdf.offset.x,
            sdf.offset.y);
    }
...
bake_font_texture(const uint8_t* font_file_content, const size_t size, const UnicodeView& codepoints)
{
    if (memcmp(
    ...
    auto sdf_bitmaps = _rasterize_sdf_bitmaps(
        font_info, font_baking_scale, codepoints
    );
...
std::tuple<bimg::ImageContainer*, BakedFontData, FontMetrics>
bake_font_texture(const File& font_file, const UnicodeView& additional_codepoints)
{
    return bake_font_texture(
        font_file.content.data(), font_file.content.size(), additional_codepoints);
}
...
FontData::FontData(const std::string& path, const UnicodeView additional_codepoints) : path(path), additional_codepoints(additional_codepoints)
{
    File file(this->path);
...
FontData::load(const std::string& path, const UnicodeView additional_codepoints)
{
    ...
    if ((font_data = _fonts_registry.get_resource({path, additional_codepoints}))) {
        return font_data;
    }

    font_data = std::shared_ptr<FontData>(new FontData(path, additional_codepoints));
    ...
    const UnicodeView text, const double scale_factor)
{
    ...
    for (UnicodeCodepoint codepoint : text) {
        ...
        if (it != this->baked_font.end()) {
            ...
            glyph_data = this->baked_font.at(static_cast<UnicodeCodepoint>('?'));
        }
...
Font
Font::load(const std::string& font_filepath, const UnicodeView additional_codepoints)
{
