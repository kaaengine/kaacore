...
typedef std::variant<std::basic_string_view<char>,
                     std::basic_string_view<char16_t>,
                     std::basic_string_view<char32_t>> UnicodeStringViewVariant;
...
class UnicodeView {
  ...
  public:
    ...
    UnicodeView();
    UnicodeView(const std::byte* data, const std::size_t length,
...
class UnicodeBuffer {
  ...
  public:
    UnicodeBuffer();
    UnicodeBuffer(const std::byte* data, const std::size_t length,
                  const UnicodeRepresentationSize representation_size);
...
struct hash<UnicodeView> {
    ...
    {
        return hash_iterable<UnicodeCodepoint, UnicodeView::iterator>(view.begin(), view.end());
    }
};
